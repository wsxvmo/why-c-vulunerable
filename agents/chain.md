...
| Pattern | Example |
|---------|---------|
| **Info leak → privilege escalation** | Leaked internal path/key enables access to restricted resource |
| **Info leak → buffer-overflow** | Leaked memory layout/addresses enable reliable ROP in a setuid binary |
| **Buffer-overflow → RCE** | Overflow in a network/setuid binary becomes code execution |
| **UAF → privilege escalation** | UAF in a privileged daemon becomes LPE |
| **path-traversal → credential leak** | File read becomes reading shadow/config secrets |
| **access-control → privilege escalation** | Missing DBus credential check becomes any-user LPE |
| **unsafe-temp-file → symlink attack** | Predictable temp file becomes overwrite of attacker-chosen file |
| **unsafe-deserialization → RCE** | Crafted pickle/yaml becomes command execution |
